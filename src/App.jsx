        ...
        <div className="shadow p-5 w-100 mx-auto">
          <table className='table'>
            ...
            {task?.map((item,index)=>{
              return (
                <tbody>
              <tr><th><img src={foto}></img></th><th>{index}</th><th>{item.name}</th><th>{item.email}</th><th>+{item.phone}</th><th>{new Date().toLocaleDateString().padStart(10, '0')}</th><th><img src={edit}></img></th><th><img src={del}></img></th></tr>
              </tbody>
              )
            })
          }
